...
%IRSSI = (
    authors     => 'Geert Hauwaerts (edited by rolle)',
...
sub away_hilight_notice {
  ...
  my $append;
  if ($away_time) {
    $append = " (awaytime: " . duration_exact(time - $away_time) . ")";
  }

        if (!$lasthilight{lc($hilight)}{'last'}) {
      	    $lasthilight{lc($hilight)}{'last'} = time();
      	    $server->command("^NOTICE $hilight Olen juuri nyt poissa ($server->{away_reason})$append, mutta olen saanut viestisi ja vastaan siihen heti kun olen taas paikalla.");
            return;
         }
